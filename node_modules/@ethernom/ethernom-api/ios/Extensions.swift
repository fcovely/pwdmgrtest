...
extension Array{
 ...
 func ParseEncryptedHeader(payload : [UInt8])->([UInt8],[UInt8], [UInt8]){
  var transportHdr = CopyBytes(payload: payload, startIdx: 0, count: UInt8( EtherTransportProtocol.ETH_BLE_HEADER_SIZE))
  let encHdr = CopyBytes(payload: payload, startIdx: UInt8(EtherTransportProtocol.ETH_BLE_HEADER_SIZE), count: UInt8(EtherTransportProtocol.ETH_BLE_ENC_HEADER_SIZE))
   let ct = UInt8(payload.count - 24)
...
func CopyBytes(payload : [UInt8], startIdx: UInt8, count : UInt8)-> [UInt8]{
  var out : [UInt8] = Array()
