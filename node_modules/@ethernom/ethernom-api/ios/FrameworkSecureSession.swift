...
class FrameworkSecureSession{
  ...
  func DoKeyExchange(pin : String){
     ...
     self._bleClient?.WriteDataToCard(data: payload, encHeader:encHeader, writeCallback: { (tvalue) in
  ...
  func doStartEncryptionWithCard(){
    ...
    self._bleClient?.WriteDataToCard(data: data, writeCallback: { (tvalue) in
  ...
  func WriteDataToCardEncrypted(doEncrypt:Bool, AppData : [UInt8]) {
    ...
    _bleClient?.WriteDataToCardEncrypted(appPort:PSD_MGR_PORT, doEncrypt: doEncrypt, EncHeaderAndData:[UInt8](epacket!),writeCallback: self.dataPacketReceived)
  }
}
