...
// **************** new encryption
  //*******************************************************************
   ...
   func getInitedPacket(payloadLength: UInt16, useEncryption: Bool) -> [UInt8]{
    ...
    return _transportProtocol.MakeTransportHeader(srcport: PSD_MGR_PORT | 0x80, destprt: PSD_MGR_PORT, control: useEncryption ? 0x81 : 0, interfaceKind: TInterface.BLE.rawValue, payloadLength: encPayloadLength,protocol: INT8NULL)
   }
  ...
  func getInitedPacket(appPort:UInt8, payloadLength: UInt16, useEncryption: Bool) -> [UInt8]{
   let encPayloadLength = payloadLength + 16
   return MakeTransportHeader(srcport: appPort | 0x80, destprt: appPort, control: useEncryption ? 0x81 : 0, interface: TInterface.BLE.rawValue, payloadLength: encPayloadLength,protocol: INT8NULL)
  }
 ...
 func WriteDataToCard(cmd : UInt8,  data : [String], encHeader: EtherEncHeader, writeCallback: @escaping ([UInt8]) -> Void) {
  ...
  func WriteDataToCardEncrypted(appPort : UInt8, doEncrypt:Bool, EncHeaderAndData : [UInt8], writeCallback: @escaping ([UInt8]) -> Void) {
    _writeCallback = writeCallback
    print("encrypted array of data outbound")
    var packet = getInitedPacket(appPort : appPort, payloadLength: UInt16(EncHeaderAndData.count - EtherTransportProtocol.ETH_BLE_ENC_HEADER_SIZE), useEncryption: doEncrypt)
